...
void main()
{
  ...
  vec2 xoverr = scale*half_window_size/r*(x_tex.xy+vec2(1.0f,-1.0f));
  /* provide gradient for texture fetching */
  vec2 dx = scale*half_window_size/r*dFdx(x_tex.xy);
  vec2 dy = scale*half_window_size/r*dFdy(x_tex.xy);
